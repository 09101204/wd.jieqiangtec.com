...
function mc_group_update() {
	...
	if(!$credit) {
		$credit = pdo_fetch('SELECT credit1, credit6 FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND uid = :uid', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']));
	}
	$credit = $credit['credit1'] + $credit['credit6'];
	...
	if($groupid > 0 && $groupid != $_W['member']['groupid']) {
		...
		mc_notice_group($_W['openid'], $_W['uniaccount']['groups'][$_W['member']['groupid']]['title'], $_W['uniaccount']['groups'][$groupid]['title']);
	}
	$_W['member']['groupid'] = $groupid;
	$_W['member']['groupname'] = $_W['uniaccount']['groups'][$groupid]['title'];
	return $_W['member']['groupid'];
}
