...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/ysxpay.php';
...
"signType" => $signType, //
		...
		// TODO 日志记录
		logResult('file in ysxpay json_encode($parameter)===' . json_encode($parameter));
		logResult('file in ysxpay $ItemUrl===' .$ItemUrl);
		include 'phpqrcode.php';
		$scanCode = QRcode::png($scanCode,false,0,5);

		//		exit;
		//		$button = '<div style="text-align:center"><input type="button" onclick="window.open(\'' . $scanCode . '\')" value="' . $GLOBALS['_LANG']['pay_button'] . '" /></div>';
		$button = "<img src='$scanCode'>";
		return $button;
	}
	...
	function respond(){
		// TODO 日志记录
		logResult('file in ysxpay json_encode($_REQUEST)===' . json_encode($_REQUEST));
		logResult('file in ysxpay json_encode($_POST)===' . json_encode($_POST));
		logResult('file in ysxpay json_encode($_GET)===' . json_encode($_GET));
		...
		// TODO 日志记录
		logResult('file in ysxpay json_encode($payment)===' . json_encode($payment));
