...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/ysxpay_wxpay.php';
...
class ysxpay_wxpay{
	...
	function get_code($order, $payment){
		 ...
		 */
		// 商户id
		$allscore_config['merchantId'] = $payment['ysxpay_wxpay_account']; // '001015013101118'
		// 安全检验码，以数字和字母组成的32位字符
		$allscore_config['key'] = $payment['ysxpay_wxpay_key']; // '22c531a0d83a46b0988ffdb038646b5b';
		...
		$allscore_config['request_gateway'] = $payment['ysxpay_wxpay_request_gateway']; // 网关地址
		$allscore_config['http_verify_url'] = $payment['ysxpay_wxpay_http_verify_url']; // http通知验证地址
...
"signType" => $signType, //
		...
		if($resultCode['reCode'] == "SUCCESS"){
			...
			if($parameter['signType'] == "RSA"){
				...
				$signFlag = verifySign($resultCode, $resultCode['sign'], $key);
				//logResult("signFlag=============".$signFlag);
			}
			//logResult("signFlag=".$signFlag);
			//验签失败
		...
		// TODO 日志记录
		logResult('file in ysxpay_wxpay json_encode($parameter)===' . json_encode($parameter));
		logResult('file in ysxpay_wxpay $ItemUrl===' .$ItemUrl);
	...
	function respond(){
		// TODO 日志记录
		logResult('file in ysxpay_wxpay json_encode($_REQUEST)===' . json_encode($_REQUEST));
		logResult('file in ysxpay_wxpay json_encode($_POST)===' . json_encode($_POST));
		logResult('file in ysxpay_wxpay json_encode($_GET)===' . json_encode($_GET));
		...
		// TODO 日志记录
		logResult('file in ysxpay_wxpay json_encode($payment)===' . json_encode($payment));
		...
		// 计算得出通知验证结果
		$allscoreNotify = new AllscoreNotifyNew($allscore_config);
